...
class Illustration2VecBase(object):
    ...
    def __init__(self, net, param_path=None, tags=None, threshold=None, mode='tag', device='cpu'):
        self.net = net
        self.mode = mode
        self.device = device
    ...
    def _estimate(self, images):
        ...
        assert(self.mode is not 'feature')
        imgs = [self._convert_image(img) for img in images]
        prob = self._extract(imgs).cpu().detach().numpy()
    ...
    def extract_feature(self, images):
        assert(self.mode is not 'tag')
        imgs = [self._convert_image(img) for img in images]
        feature = self._extract(imgs)
    ...
    def change_mode(self, new_mode='tag'):
        self.mode = new_mode
        self.net._change_mode(new_mode=new_mode)
